...
class TokenContractTestHelper {
  ...
  static async localDeploy(deployerKey: PrivateKey) {
    ...
    }, [deployerKey, TokenContractTestHelper.zkAppPrivateKey]);
  }

  static async executeTransaction(
    action: (deployerAccount: PublicKey) => void,
    signers: PrivateKey[],
    fundNewAccount: boolean = false,
    attachZkApp: boolean = false
  ) {
    const txn = await Mina.transaction(TokenContractTestHelper.local.testAccounts[0].publicKey, () => {
      if (fundNewAccount) AccountUpdate.fundNewAccount(TokenContractTestHelper.local.testAccounts[0].publicKey);
      action(TokenContractTestHelper.local.testAccounts[0].publicKey);
      if (attachZkApp) {
        AccountUpdate.attachToTransaction(TokenContractTestHelper.zkApp.self);
        TokenContractTestHelper.zkApp.requireSignature();
      }
    });
    await txn.prove();
...
describe('TokenContract', () => {
  ...
  it('should deploy the TokenContract with initial totalAmountInCirculation', async () => {
    await TokenContractTestHelper.localDeploy(TokenContractTestHelper.local.testAccounts[0].privateKey);
  ...
  it('should mint tokens and update totalAmountInCirculation correctly', async () => {
    ...
    }, [TokenContractTestHelper.local.testAccounts[0].privateKey, TokenContractTestHelper.zkAppPrivateKey], true, true);
  ...
  it('should burn tokens and update totalAmountInCirculation correctly', async () => {
    ...
    }, [TokenContractTestHelper.local.testAccounts[0].privateKey, TokenContractTestHelper.local.testAccounts[1].privateKey, TokenContractTestHelper.zkAppPrivateKey], false, true);
  ...
  it('should transfer tokens and update balances correctly', async () => {
    ...
    }, [TokenContractTestHelper.local.testAccounts[0].privateKey, TokenContractTestHelper.local.testAccounts[1].privateKey, TokenContractTestHelper.zkAppPrivateKey], true, true);
    ...
    }, [TokenContractTestHelper.local.testAccounts[0].privateKey, TokenContractTestHelper.local.testAccounts[1].privateKey, TokenContractTestHelper.local.testAccounts[2].privateKey, TokenContractTestHelper.zkAppPrivateKey], true, true);
